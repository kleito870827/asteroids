...
function gameLoop() {
    ...
    if (varControl.upPressed == 1) ySpeed = Math.max(ySpeed - 1, -1 * maxSpeed);
    if (varControl.downPressed == 1) ySpeed = Math.min(ySpeed + 1, 1 * maxSpeed);
    if (varControl.rightPressed == 1) xSpeed = Math.min(xSpeed + 1, 1 * maxSpeed);
    if (varControl.leftPressed == 1) xSpeed = Math.max(xSpeed - 1, -1 * maxSpeed);

    // deceleration
    if (varControl.upPressed == 0 && varControl.downPressed == 0) slowDownY();
    if (varControl.leftPressed == 0 && varControl.rightPressed == 0) slowDownX();
